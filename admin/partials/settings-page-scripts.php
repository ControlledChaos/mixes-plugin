...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'General', 'mixes-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Vendor', 'mixes-plugin' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'mmp_tabs_script_options', $tabs );
...
if ( 'general' == $active_tab ) {
    $section = 'mmp-scripts-general';
    $fields  = 'mmp-scripts-general';
} elseif ( 'vendor' == $active_tab ) {
    $section = 'mmp-scripts-vendor';
    $fields  = 'mmp-scripts-vendor';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'mmp_section_script_options', $section );
$do_fields  = apply_filters( 'mmp_fields_script_options', $fields );
 ...
 */
if ( 'general' == $active_tab  ) {
    $save = __( 'Save General', 'mixes-plugin' );
} elseif ( 'vendor' == $active_tab ) {
    $save = __( 'Save Vendor', 'mixes-plugin' );
} else {
    $save = __( 'Save Settings', 'mixes-plugin' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'mmp_save_script_options', $save );

?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e( 'Script Options', 'mixes-plugin' ); ?></h1>
    <?php if ( is_rtl() ) : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper left.', 'mixes-plugin' ); ?></p>
    <?php else : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper right.', 'mixes-plugin' ); ?></p>
    <?php endif; ?>
