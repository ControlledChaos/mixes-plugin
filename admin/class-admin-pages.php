...
class Admin_Pages {
    ...
    public function __construct() {
        ...
        if ( mmp_acf_options() ) {
            ...
            $admin_header = get_field( 'mmp_use_admin_header', 'option' );
        } else {

            // Otherwise look for the WP API setting.
            $admin_header = get_option( 'mmp_use_admin_header' );
        }
    ...
    public function about_plugin() {
        ...
        if ( mmp_acf_options() ) {
            ...
            // Get the field.
            $link_label = get_field( 'mmp_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress/ClassicPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'mmp_site_plugin_link_label' ) );
        }
        ...
        // Otherwise use Site Plugin as the label.
            ...
            $label = __( 'Site Plugin', 'mixes-plugin' );
        }
        ...
        if ( mmp_acf_options() ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'mmp_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress/ClassicPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'mmp_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'mixes-plugin' );
        }
    ...
    public function about_plugin_output() {
        ...
        require MMP_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'mixes-plugin' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once MMP_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once MMP_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'mixes-plugin' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'mixes-plugin' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            '<p>%1s <br /><a href="%2s" target="_blank">%3s</a> <br />%4s</p>',
            __( 'Visit:', 'mixes-plugin' ),
            'http://ccdzine.com/',
            'Controlled Chaos Design',
            __( 'for more free downloads.', 'mixes-plugin' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'mixes-plugin' )
         );
	...
	public function admin_menus() {
		...
		register_nav_menus( [
			'admin-header' => __( 'Admin Header Menu', 'mixes-plugin' )
		] );
	...
	public function admin_header() {
		...
		if ( ! empty( $admin_header ) ) {
			...
			include_once MMP_PATH . 'admin/partials/admin-header.php';
		}
	...
	public function admin_header_layout() {
        ...
        $style .= '.gutenberg-editor-page .mmp-admin-header { padding-left: 20px; }';
        $style .= '.gutenberg-editor-page .mmp-admin-header:before { content: ""; display: block; width: 100%; height: 56px; }';

        // End the style block.
        $style .= '</style>';

        // Apply a filter for custom admin themeing.
        $style = apply_filters( 'mmp_admin_header_layout', $style );
    ...
    public function title_placeholders( $title ) {
        ...
        if ( 'post' == $screen->post_type ) {
            $post_title = esc_html__( 'Post Title', 'mixes-plugin' );

        // Post type: page.
        } elseif ( 'page' == $screen->post_type ) {
            $post_title = esc_html__( 'Page Title', 'mixes-plugin' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'mixes-plugin' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'mmp_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'mmp_column_thumbnail_size', $size );
