...
class Frontend {
	...
	public function __construct() {
		...
		// Get inline options.
		$jquery  = get_option( 'mmp_inline_jquery' );
	...
	public function dependencies() {
		...
		// Get inline options.
		$scripts = get_option( 'mmp_inline_scripts' );
		$styles  = get_option( 'mmp_inline_styles' );

		// Add styles inline if option selected.
		if ( $styles ) {
			require_once MMP_PATH . 'frontend/class-styles-inline.php';

		// Otherwise enqueue styles.
		} else {
			require_once MMP_PATH . 'frontend/class-styles-enqueue.php';
		}

		// Add scripts inline if option selected.
		if ( $scripts ) {
			require_once MMP_PATH . 'frontend/class-scripts-inline.php';

		// Otherwise enqueue scripts.
		} else {
			require_once MMP_PATH . 'frontend/class-scripts-enqueue.php';
		}
	...
	public function get_jquery() {
		...
		if ( ! is_customize_preview() ) {
			...
			$jquery = file_get_contents( MMP_PATH . '/assets/js/jquery.min.js' );
	...
	public function attachment_fancybox() {
		...
		if ( is_attachment() && get_option( 'mmp_enqueue_fancybox_script' ) ) { ?>
