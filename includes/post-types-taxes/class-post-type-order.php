...
if ( mmp_acf_options() ) {
    $sort_order = get_field( 'mmp_use_custom_sort_order', 'option' );

// If Advanced Custom Fields is not active.
} else {
    $sort_order = get_option( 'mmp_use_custom_sort_order' );
}
...
class Post_Types_Taxes_Order {
	...
	public static function instance() {
		...
		if ( is_null( $instance ) ) {
            ...
            if ( ! get_option( 'mmp_order_install ' ) ) {
    ...
    public function order_install() {
        ...
        update_option( 'mmp_order_install', 1 );
    ...
    public function admin_menu() {
        ...
        add_options_page(
            __( 'Posts & Taxonomies Sort Order', 'mixes-plugin' ),
            __( 'Sort Order', 'mixes-plugin' ),
            'manage_options',
            'sort-order-settings',
            [ $this, 'admin_page' ]
        );
    ...
    public function admin_page() {
        ...
        require MMP_PATH . 'admin/partials/settings-page-posts-order.php';
    ...
    public function load_script_css() {
        ...
        if ( $this->_check_load_script_css() ) {
            ...
            wp_enqueue_script( 'post-tax-order', MMP_URL . 'admin/assets/js/post-tax-order.js', [ 'jquery' ], null, true );
    ...
    public function update_options() {
        ...
        if ( ! isset( $_POST['mmp_posts_order_submit'] ) ) {
            return false;
        }

        check_admin_referer( 'mmp_posts_order_nonce' );

        $input_options            = [];
        $input_options['objects'] = isset( $_POST['objects'] ) ? $_POST['objects'] : '';
        $input_options['tags']    = isset( $_POST['tags'] ) ? $_POST['tags'] : '';

        update_option( 'mmp_order_options', $input_options );
    ...
    public function get_order_options_objects() {
        ...
        if ( $mmp_order_options = get_option( 'mmp_order_options' ) ) {
            $mmp_order_options = get_option( 'mmp_order_options' );
        } else {
            $mmp_order_options = [];
        }

        if ( isset( $mmp_order_options['objects'] ) && is_array( $mmp_order_options['objects'] ) ) {
            $objects = $mmp_order_options['objects'];
        } else {
            $objects = [];
        }
    ...
    public function get_order_options_tags() {
        ...
        if ( isset( $mmp_order_options['tags'] ) && is_array( $mmp_order_options['tags'] ) ) {
            $tags = $mmp_order_options['tags'];
        } else {
            $tags = [];
        }
 ...
 */
function mmp_post_types_taxes_order() {
...
function uninstall_db() {
    ...
    delete_option( 'mmp_order_install' );
