...
function themedrive_add_pages() {
    add_theme_page(
        ...
        __( 'Theme Test', 'mixes-plugin' ),
        'edit_theme_options',
        MMP_ADMIN_SLUG . '-theme-test',
...
function themedrive_switcher() {
    ...
    if ( themedrive_is_enabled() ) {
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is enabled with', 'mixes-plugin' ),
            $theme_name
        );
    } else {
        $get_theme  = wp_get_theme();
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is disabled', 'mixes-plugin' ),
            __( 'The active theme is', 'mixes-plugin' ),
            $theme_name
        );
    }
...
function themedrive_options_page() {
    ...
    if ( isset( $_POST['button'] ) && 'Enable Theme Drive' == $_POST['button'] ) {
        ...
        $msg_status = sprintf(
            ...
            __( 'Live theme test is enabled with', 'mixes-plugin' ),
            $get_theme_name,
            __( 'theme', 'mixes-plugin' ),
            esc_url( site_url() ),
            __( 'View Site', 'mixes-plugin' )
        );
        ...
        $msg_status = __( 'Live theme test has been disabled.', 'mixes-plugin' );
...
?>
<div class="wrap" >
    <h2><?php _e( 'Live Theme Test', 'mixes-plugin' ); ?></h2>
    <?php $action_url; ?>
    ...
    <?php wp_nonce_field( 'theme-drive' ); ?>
        <h2><?php _e( 'Instructions', 'mixes-plugin' ); ?></h2>
        <ol>
            <li><?php _e( 'Select a theme to preview live on the site from the select box below (lists all installed themes).', 'mixes-plugin' ); ?></li>
            <li><?php _e( 'Enable theme test', 'mixes-plugin' ); ?></li>
            <li><?php _e( 'Once the theme is ready to go live, disable theme test and activate the theme on the Themes page.', 'mixes-plugin' ); ?></li>
        </ol>
   	    <p><?php _e( 'Additionally you may add "?theme=xxx" to a URL, where xxx is the slug of the theme you want to test.', 'mixes-plugin' ); ?></p>
        <?php themedrive_switcher(); ?>
        <h2><?php _e( 'User Role', 'mixes-plugin' ); ?></h2>
        <p><?php _e( 'Specify the level of users to have access to the selected theme preview.', 'mixes-plugin' ); ?></p>
        <p><?php _e( 'By default it is set to 10 (admin only). Level 7 are editors, level 4 are authors and level 1 are contributors.', 'mixes-plugin' ); ?></p>
        <p><?php _e( 'The access level is ignored for accessing the site with ?theme=xxx paramaeter.', 'mixes-plugin' ); ?></p>
        <p><input style="border-width: inherit; border-style: inherit; border-color: inherit; width: 30px;" name="access_level" id="access_level" value="<?php echo esc_attr( $access_level ); ?>" /> <label for="access_level"><?php _e( 'Access level', 'mixes-plugin' ); ?></label></p>
        <p class="submit">
