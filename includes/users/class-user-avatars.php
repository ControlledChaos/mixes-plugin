...
class User_Avatars {
	...
	public function __construct() {
		...
		// Shortcode.
		add_shortcode( 'mixes-plugin', [ $this, 'shortcode' ] );
	...
	public function admin_init() {
		...
		add_settings_field(
			...
			__( 'Local Avatar Permissions',	'mixes-plugin' ),
			[ $this, 'avatar_settings_field' ],
			'discussion',
			'avatars',
			[ esc_html__( 'Only allow users with file upload capabilities to upload local avatars (Authors and above).', 'mixes-plugin' ) ]
		);

		register_setting(
			'discussion',
			'mmp_user_avatars_caps'
		);
	...
	public function avatar_settings_field( $args ) {
		...
		$option = get_option( 'mmp_user_avatars_caps' );

		$html = '<p><input type="checkbox" id="mmp_user_avatars_caps" name="mmp_user_avatars_caps" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="mmp_user_avatars_caps"> ' . $args[0] . '</label></p>';
	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 96, $default = '', $alt = false ) {
		...
		$local_avatars = get_user_meta( $user_id, 'mmp_user_avatar', true );
		...
		if ( empty( $local_avatars[$size] ) ) {
			...
			update_user_meta( $user_id, 'mmp_user_avatar', $local_avatars );
		...
		return apply_filters( 'mmp_user_avatar', $avatar );
	...
	public function edit_user_profile( $profileuser ) {
		...
		<h3><?php _e( 'Avatar', 'mixes-plugin' ); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="basic-user-avatar"><?php _e( 'Upload Avatar', 'mixes-plugin' ); ?></label></th>
				<td style="width: 50px;" valign="top">
					<?php echo get_avatar( $profileuser->ID ); ?>
				</td>
				<td>
				<?php
				$options = get_option( 'mmp_user_avatars_caps' );
				if ( empty( $options['mmp_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					// Nonce security ftw.
					wp_nonce_field( 'mmp_user_avatar_nonce', '_mmp_user_avatar_nonce', false );

					// File upload input.
					echo '<input type="file" name="basic-user-avatar" id="basic-local-avatar" /><br />';

					if ( empty( $profileuser->mmp_user_avatar ) ) {
						echo '<span class="description">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', 'mixes-plugin' ) . '</span>';
					} else {
						echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" /> ' . __( 'Delete local avatar', 'mixes-plugin' ) . '<br />';
						echo '<span class="description">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'mixes-plugin' ) . '</span>';
					}

				} else {
					if ( empty( $profileuser->mmp_user_avatar ) ) {
						echo '<span class="description">' . __( 'No local avatar is set. Set up your avatar at Gravatar.com.', 'mixes-plugin' ) . '</span>';
					} else {
						echo '<span class="description">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', 'mixes-plugin' ) . '</span>';
					}
				}
				?>
				</td>
	...
	public function edit_user_profile_update( $user_id ) {
		...
		if ( ! isset( $_POST['_mmp_user_avatar_nonce'] ) || ! wp_verify_nonce( $_POST['_mmp_user_avatar_nonce'], 'mmp_user_avatar_nonce' ) )
			return;
		...
		if ( ! empty( $_FILES['basic-user-avatar']['name'] ) ) {
			...
			update_user_meta( $user_id, 'mmp_user_avatar', [ 'full' => $avatar['url'] ] );
	...
	function shortcode() {
		...
		<form id="basic-user-avatar-form" action="<?php the_permalink(); ?>" method="post" enctype="multipart/form-data">
			...
			if ( empty( $options['mmp_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
				...
				if ( empty( $profileuser->mmp_user_avatar ) ) {
					echo '<p class="description">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', 'mixes-plugin' ) . '</p>';
				} else {
					echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" /> ' . __( 'Delete local avatar', 'mixes-plugin' ) . '<br />';
					echo '<p class="description">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'mixes-plugin' ) . '</p>';
				}

			} else {
				if ( empty( $profileuser->mmp_user_avatar ) ) {
					echo '<p class="description">' . __( 'No local avatar is set. Set up your avatar at Gravatar.com.', 'mixes-plugin' ) . '</p>';
				} else {
					echo '<p class="description">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', 'mixes-plugin' ) . '</p>';
				}
			}
			?>
			<input type="submit" name="manage_avatar_submit" value="<?php _e( 'Update Avatar', 'mixes-plugin' ); ?>" />
		</form>
		<?php
	...
	public function bbpress_user_profile() {
		...
		echo '<div>';
			echo '<label for="basic-local-avatar">' . __( 'Avatar', 'mixes-plugin' ) . '</label>';
 			echo '<fieldset class="bbp-form avatar">';
				...
				if ( empty( $options['mmp_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					...
					if ( empty( $profileuser->mmp_user_avatar ) ) {
						echo '<span class="description" style="margin-left:0;">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', 'mixes-plugin' ) . '</span>';
					} else {
						echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" style="width:auto" /> ' . __( 'Delete local avatar', 'mixes-plugin' ) . '<br />';
						echo '<span class="description" style="margin-left:0;">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'mixes-plugin' ) . '</span>';
					}

				} else {
					if ( empty( $profileuser->mmp_user_avatar ) ) {
						echo '<span class="description" style="margin-left:0;">' . __( 'No local avatar is set. Set up your avatar at Gravatar.com.', 'mixes-plugin' ) . '</span>';
					} else {
						echo '<span class="description" style="margin-left:0;">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', 'mixes-plugin' ) . '</span>';
					}
				}
	...
	public function avatar_defaults( $avatar_defaults = [] ) {
		...
		if ( get_option( 'mmp_block_gravatar' ) ) {
			$new_avatar_defaults = [
				'mystery' => __( 'Mystery Person', 'mixes-plugin' ),
				'blank'   => __( 'Blank', 'mixes-plugin' )
			];
		}
	...
	public function avatar_delete( $user_id ) {
		...
		$old_avatars = get_user_meta( $user_id, 'mmp_user_avatar', true );
		$upload_path = wp_upload_dir();
		...
		delete_user_meta( $user_id, 'mmp_user_avatar' );
